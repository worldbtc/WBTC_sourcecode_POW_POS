...
int64_t GetProofOfWorkReward(int64_t nFees)
{
	...
	if (nBestHeight > THE_FORK_HEIGHT)
		nSubsidy = 2 * COIN;			// reduce to 2 @ block 215000
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
	...
	if (nBestHeight > THE_FORK_HEIGHT)
		nRewardCoinYear = COIN_YEAR_REWARD_NEW; // Increase to 50% @ block 215000
...
static unsigned int GetNextTargetRequiredV1(const CBlockIndex* pindexLast, bool fProofOfStake)
{
	...
	// Don't allow zero or negative value after block 215000
	if (pindexLast->nHeight > THE_FORK_HEIGHT && nActualSpacing < 1)
		nActualSpacing = 1;
